...
# FIRST VISUALIZATION CODE: MAP VISUALIZATION (HARUKA)
hood_data <- data_bike %>%
  group_by(Hood_ID) %>%
  count()
...
register_google(key='AIzaSyBz4MCpBk9BphLx0YOuV99PTf9vq7MLbkY')
...
<br>
From examining the map, we can visually observe that most parts of  Toronto do not have a major percentage of bike thefts. However, we do see a concentration near the most southern ends of Toronto. Let’s compare the 140 neighborhoods in the most recent year 2019 on a large scale to see how it is distributed individually. <br>
<br>
```{r, warning = FALSE, message=FALSE, echo=FALSE}
# SECOND VISUALIZATION: PARETO VISUALIZATION (ARUSAN)
...
pareto_vis <- ggplot(data1, aes(x = reorder(Neighbourhood, -Thefts))) +
  ...
  geom_label(data = pointlabels[1:2,], label = pointlabels$label[1:2], 
    ...
    color = "black", fill=c("#619CFF", "#00BA38"),
    vjust = 1, hjust = 0) +
...
<br>
```{r, warning = FALSE, message=FALSE, echo=FALSE}
# THIRD VISUALIZATION: NEIGHBORHOOD VISUALIZATION (ARUSAN)
...
data.wide <- spread(data3, Occurrence_Year, Thefts)
...
data2 %>% filter(Premise %in% c("Apartment", "House", "Outside")) %>% 
...
```{r, warning = FALSE, message=FALSE, echo=FALSE}
# FIFTH VISUALIZATION: PERCENTAGE BREAKDOWN VISUALIZATION (HARUKA)
...
```{r, warning = FALSE, message=FALSE, echo=FALSE}
# SIXTH VISUALIZATION: HEATMAP VISUALIZATION (HARUKA)
time_vec <- c()
